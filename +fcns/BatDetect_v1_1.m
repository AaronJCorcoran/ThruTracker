function dets = BatDetect_v1_1(app, opts)
        ...
        dets.clouds = nan(endFrame,1);
            for i=startFrame:endFrame
                ...
                BW = bg>60/255;             % Fixed threshold is intentionally low to get turbine blades at odd angles
                                            % This will be a problem if
                                            % cloudy!
                if opts.subtractBackground
                    BW(c>opts.thresh) = 1;               % Add moving parts
                end
                ...
                if app.opts.subtractBackground
                    bwB=bg>60/255 | c> opts.thresh;     % Could adjust this to do a rolling scan for background subtraction
                else
                    bwB=bg>60/255;
                end
                dets.clouds(i) = sum(sum(bwB));
                ...
                if numel(idxd)>0 && numel(idxd)<4 % anything above 4 detections is likely bad background & noise
                    dets.uvdata=[dets.uvdata;[idxd*0+i,idxd*0+app.v.CurrentTime,stats2.Centroid(idxd,:)]];
                end
