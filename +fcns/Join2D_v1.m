function [out] = Join2D_v1(dets,opts,startFrame,endFrame)

% function [out] = birdJoin3D_v1(birds,opts)
%
% Uses MATLAB's assignDetectionsToTracks in 2D to try and track the
% points animals in a scene
%
% dets.uvdata has detection data
% 
% Join2D_v1 is modified from birdJoin3D_v1, written by Dr. Tyson Hedrick

% control whether detailed status messages are printed or not
msgs=false;

%Allow option for specifying start and end frames
if nargin > 2
    opts.startFrame = startFrame;
    opts.endFrame = endFrame;
end
...
if opts.minimumTrackLength==0
  ...
  pruning=true;        %i.e., delete tracks shorter than minimumTrackLength
end
...
foo=configureKalmanFilter('ConstantVelocity',ones(1,dets.numCams*2)*2,initialEstimateErrorXY,motionNoiseXY,measurementNoiseXY);
...
for i=1:numTracks
  ...
  Axy2(:,:,i)=Axy;
  Qxy2(:,:,i)=Qxy;
  Xxy2(1:2:dets.numCams*4,1,i)=xy(i,:);
...
for k=pseq
  ...
  fooXY=inf(numel(activeTracks),dets.numCams*2);
  fooXYd=inf(numel(activeTracks),dets.numCams*2);
  for i=1:numel(activeTracks)
    ...
    % copy predictions
    fooXY(i,:)=Xxy2(1:2:dets.numCams*4,:,activeTracks(i)); % kalman xy coordinates
    fooXYd(i,:)=Xxy2(2:2:dets.numCams*4,:,activeTracks(i)); % kalman xy velocity
  ...
  if numel(dets.xypts{k+1})>0
    ...
    % assignment
    if sum(xyz)==0
        [assignments,unassignedTracks,unassignedDetections] = fcns.tileAssignXY_v1(xy,fooXY,trackNewness(activeTracks),trackLength(activeTracks),opts,fooXYd);
    else
  ...
  if msgs
    ...
    % xy
    idxT=repmat([activeTracks(assignments(:,1))*tBlock-(tBlock-5)],1,dets.numCams*2);
    if numel(idxT)>0
      for i=2:dets.numCams*2
        idxT(:,i)=idxT(:,i-1)+1;
      end
      idxT=reshape(idxT',1,numel(idxT));
      idxX=repmat([assignments(:,2)*dets.numCams*2-(dets.numCams*2-1)],1,dets.numCams*2);
      for i=2:dets.numCams*2
        idxX(:,i)=idxX(:,i-1)+1;
      end
      idxX=reshape(idxX',1,numel(idxX));
    ...
    if numel(assignments)>0
      for i=activeTracks(assignments(:,1))'
        ...
        Pxy2(:,:,i)=(eye(dets.numCams*4)-Kxy2(:,:,i)*Hxy2(:,:,i))*Pxy2(:,:,i);
    ...
    for i=1:numel(unassignedDetections)
      ...
      out(k+1,numTracks*tBlock-(tBlock-1):numTracks*tBlock)=[xyz(unassignedDetections(i),1:4),xy(unassignedDetections(i),1:dets.numCams*2)];
      ...
      Axy2(:,:,numTracks)=Axy;
      Qxy2(:,:,numTracks)=Qxy;
      Xxy2(1:2:dets.numCams*4,1,numTracks)=full(out(k+1,numTracks*tBlock-(tBlock-5):numTracks*tBlock-0));
  ...
  % ajc added:
    ...
    if size(dets.uvData{1},2) > 6
        for i = 1:size(dets.uvData,2)                                    %Cycle through each camera
            uvd = dets.uvData{i};     
...
% trim padding
idx=find(sum(out(:,5:tBlock:end))==0);
for i=fliplr(idx)
  out(:,i*tBlock-(tBlock-1):i*tBlock)=[];
end
