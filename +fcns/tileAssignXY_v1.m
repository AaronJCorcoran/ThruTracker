function [assignments,unassignedTracks,unassignedDetections] = tileAssignXY_v1(xy,fooXY,trackNewness,trackLength,opts, fooXYd)
...
for i=1:size(tbreaks,1)
  ...
  if numel(idx1)>0 & numel(idx2)>0
    ...
    %% Need to Check this for multiple tracks
    speed_thresh = 2*abs(fooXYd(:,2));
    badMatchThreshold = repmat(opts.badMatchThreshold,size(fooXYd,1),1);
    idx = find(badMatchThreshold < speed_thresh);
    for j = 1:length(idx)
        badMatchThreshold(idx(j)) = speed_thresh(idx(j));
    end
    ...
    cost(cost>badMatchThreshold)=Inf;
    % get assignments
    as=[];
    as(:,1)=1:numel(idx2);
    [as(:,2)]=fcns.assignmentoptimal(cost);
