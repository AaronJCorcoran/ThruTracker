function [out] = Join2D_v1(dets,opts)
...
xy=birds.xypts{frame}(:,:);     %NEED TO CONVERT UVDATA TO XY FORMAT
tBlock = size(xy,2);            % size of an xy track array
%tBlock=size(xyz,2)+size(xy,2); % size of an [xyz,xy] track array

% setup output arrays
out(frame,:)=reshape([xy]',1,numel([xy]));
